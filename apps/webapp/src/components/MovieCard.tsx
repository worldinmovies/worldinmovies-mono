...
const injectMovieSchema = (movie: Movie) => {
  const schema = {
  ...
  let scriptTag = document.getElementById(`movie-schema-${movie.id}`);
  if (!scriptTag) {
    scriptTag = document.createElement('script');
    scriptTag.setAttribute('type', 'application/ld+json');
    scriptTag.setAttribute('id', `movie-schema-${movie.id}`);
...
export const MovieCard = ({ movie, onClick }: MovieCardProps) => {
  useEffect(() => {
    injectMovieSchema(movie);
    return () => {
      const tag = document.getElementById(`movie-schema-${movie.id}`);
      if (tag) tag.remove();
    };
  }, [movie]);
  ...
  return (
    <Card 
      className="group relative overflow-hidden bg-gradient-card border-border hover:border-primary/50 transition-all duration-500 hover:scale-105 hover:shadow-cinematic cursor-pointer"
      onClick={onClick}
    >
      {/* Movie Poster */}
      <div className="relative aspect-[2/3] overflow-hidden">
        {!imageError ? (
          ...
          <div className="w-full h-full bg-gradient-card flex items-center justify-center">
            <div className="text-center p-4">
              ...
              <div className="text-sm text-muted-foreground">No poster available</div>
            </div>
          </div>
        )}
        ...
        <div className="absolute top-3 left-3 bg-black/70 backdrop-blur-sm rounded-full px-2 py-1 flex items-center gap-1">
          <span className="text-sm">{movie.countryFlag && (
            <img
